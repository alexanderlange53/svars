...
test_that("unrestricted id.cv estimation with 3-dim works and 2 regimes", {
  ...
  set.seed(23211)
  v1 <- vars::VAR(USA, p = 6, ic = "AIC" )
  x1 <- id.cv(v1, SB = 59, SB2 = 110)
  x2 <- id.cv_boot(v1, SB = 59, SB2 = 110)

  expect_equal(round(x1$Lik, 4), round(x2$Lik, 4))
  expect_equal(round(sum(diag(x1$Lambda)),4),  round(sum(diag(x2$Lambda)),4))
  expect_equal(round(sum(x1$B),4), round(sum(x2$B),4))

  expect_equal(round(sum(x1$Lambda_SE), 4), 1.4242, tolerance = 0.002)
...
test_that("unrestricted id.cv estimation with 3-dim works with constant +  trend and 2 regimes", {
  ...
  expect_equal(round(sum(x1$Lambda_SE), 4), 1.4281, tolerance = 0.002)
...
test_that("unrestricted id.cv estimation with 3-dim works without any determininstic term and 2 regimes", {
  ...
  expect_equal(round(sum(x1$Lambda_SE), 4), 1.3996, tolerance = 0.002)
...
test_that("unrestricted id.cv and boot estimation with 2-dim works and 2 regimes", {
  ...
  expect_equal(round(sum(x1$Lambda_SE), 4), 0.326, tolerance = 0.002)
...
test_that("restricted id.cv and boot estimation with 3-dim works and 2 regimes", {
  ...
  x2 <- id.cv_boot(v1, SB = 59, SB2 = 110, restriction_matrix = restmat)
  expect_equal(round(x1$Lik, 4), round(x2$Lik, 4))
  expect_equal(round(sum(diag(x1$Lambda)),4), round(sum(diag(x2$Lambda)),4))
  expect_equal(round(sum(x1$B),4),round(sum(x2$B),4))

  expect_equal(round(sum(x1$Lambda_SE), 4), 1.4106, tolerance = 0.002)
...
test_that("restricted id.cv and boot estimation with 2-dim works and 2 regimes", {
  ...
  expect_equal(round(sum(x1$Lambda_SE), 4), 0.309)
...
test_that("unrestricted id.cv and boot estimation with 5-dim works and 2 regimes", {
  ...
  set.seed(23211)
  v1 <- vars::VAR(LN, p = 3)
  x1 <- id.cv(v1, SB = 250, SB2 = 400)
  x2 <- id.cv_boot(v1, SB = 250, SB2 = 400)

  expect_equal(round(x1$Lik, 4), round(x2$Lik, 4))
  expect_equal(round(sum(diag(x1$Lambda)),4),round(sum(diag(x2$Lambda)),4))
  expect_equal(round(sum(x1$B),4),round(sum(x2$B),4))

  expect_equal(round(sum(x1$Lambda_SE), 4), 0.3606, tolerance = 0.002)
...
test_that("restricted id.cv and boot estimation with 5-dim works and 2 regimes", {
  ...
  x1 <- id.cv(v1, SB = 250, SB2 = 400, restriction_matrix = restmat)
  x2 <- id.cv_boot(v1, SB = 250, SB2 = 400, restriction_matrix = restmat)

  expect_equal(round(x1$Lik, 4), round(x2$Lik, 4))
  expect_equal(round(sum(diag(x1$Lambda)),4),round(sum(diag(x2$Lambda)),4))
  expect_equal(round(sum(x1$B),4),round(sum(x2$B),4))

  expect_equal(round(sum(x1$Lambda_SE), 4), 0.3419, tolerance = 0.002)
