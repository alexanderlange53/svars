...
test_that("Luetkepohl Netsunajev 5*5 example", {
  ...
  expect_equal(round(likelihood_st(parameter = c(B, diag(Lambda)), u_t = u_t, G = G_grid, k = 5, Tob = 447), 3),
                     2976.656)
})
...
test_that("Random 2*2 example works", {
  ...
  expect_equal(round(likelihood_st(parameter = c(B, diag(Lambda)), u_t = u_t, G = G_grid, k = 2, Tob = 200), 3),
               906.717)
})
